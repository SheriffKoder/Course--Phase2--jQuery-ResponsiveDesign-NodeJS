...
exports.postAddProduct = (req, res, next) => {
    ...
    //(20.0.3)
    //if no image (multer found a different file type)
    ...
    //image will output image.path which we need to fetch 
    //the image from the fs
    //console.log(image);
    //(20.0.3)
    //store the image path on the fs to imageUrl
...
exports.postEditProduct = (req,res, next) => {
    ...
    //const updatedImageUrl = req.body.imageUrl; //-(20.0.4)
    const image = req.file;  //(20.0.4)
    const updatedPrice = req.body.price;
    const updatedDescription = req.body.description;

    //if image is undefined (no image saved)
    //keep the old file
    ...
    if (!errors.isEmpty()) {
        ...
        return res.status(422).render("admin/edit-product", 
        {
            ...
            product: {
                title: updatedTitle,
                //imageUrl: updatedImageUrl, //-(20.0.4) we do not need that
                price: updatedPrice,
...
//exports.postDeleteProduct = (req, res, next) => { //-(22.0.2)
exports.deleteProduct = (req, res, next) => {
    //const prodId = req.body.productId; 
    //the delete verb in the router does not have a body
    //so we will use the params set in the router path
    const prodId = req.params.productId; //(22.0.2)
    ...
    */
	...
	.then((result) => {
        ...
        //res.redirect("/admin/products"); //-(22.0.2)
        //(22.0.2)
        //will not redirect any more, will not load a new page, will change the dom
        //as we do not redirect, be clear about the status code we have
        //even though its the default 200
    ...
    .catch((err) => {
        // console.log(err);
        //return next(new Error(err).httpStatusCode=500); //(19.0.3) //-(22.0.2)
        //(22.0.2)
        //will also return a response with status 500
        res.status(500).json({
